window.app.controller("ListController", [
  ...
  function ($scope, $rootScope, $http, Socket) {
    ...
    var getHash = function () {
      ...
      var hashes = window.location.pathname.match(
        new RegExp("(W|^)(" + words + ")(W|$)"),
      );
      if (hashes) {
        return hashes[0];
      } else {
    ...
    $scope.update = function () {
      $http
        .put("/api/" + $scope.model + "/" + $scope.id, $scope.edit)
        .success(function () {
          $scope.alert = "Message: " + $scope.model + " has been update";
        })
        .error(function (data, status) {
          if (status === 401) {
            $scope.error = "Update rejected.";
            return false;
          }
          if (data.error) {
            $scope.error = data.error;
    ...
    $scope.delete = function () {
      $http
        .delete("/api/" + $scope.model + "/" + $scope.id)
        .success(function () {
